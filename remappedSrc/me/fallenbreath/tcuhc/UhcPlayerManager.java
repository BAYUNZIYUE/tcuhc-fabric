	...
	public void onPlayerDeath(ServerPlayerEntity player, DamageSource cause) {
		if (gameManager.isGamePlaying()) {
			...
			if (combatPlayerList.contains(gamePlayer) && gamePlayer.isAlive()) {
				...
				player.method_48926().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ENTITY_LIGHTNING_BOLT_THUNDER, SoundCategory.PLAYERS, 10000.0F, 0.8F + UhcGameManager.rand.nextFloat() * 0.2F);
			}
		}
